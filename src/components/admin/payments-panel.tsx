...
export function PaymentsPanel({ payments, students }: Props) {
  ...
  const debtors = students.filter(
    (s) => Number(s.total_amount) - Number(s.discount) - Number(s.paid_amount) > 0
  );
    ...
    <div className="grid gap-8 lg:grid-cols-2">
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="font-semibold text-slate-900">To‘lov kuni yaqinlar / qarzdorlar</h2>
          <ul className="mt-4 max-h-[280px] space-y-3 overflow-y-auto text-sm">
            {debtors.map((s) => {
                ...
                <li key={s.id} className="flex items-center justify-between border-b border-slate-100 pb-2">
                  <div>
                    ...
                    <div className="text-xs text-slate-500">
                      To‘lov kuni: {s.payment_due_date || "kiritilmagan"}
                    </div>
                  </div>
                  <div className="text-right text-xs">
                    <div className="text-red-700">Qarz: {formatUzs(due)}</div>
                    <div className="text-slate-500">{s.payment_status}</div>
                  </div>
                </li>
              );
            })}
            {debtors.length === 0 && <li className="text-slate-500">Qarzdor o‘quvchi yo‘q</li>}
          </ul>
        </div>
      </div>
